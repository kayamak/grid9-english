...
export class PatternGenerator {
  static generate(
    ...
    if (verbType === 'be') {
      ...
      rawSentence = this.generateBeVerb(
        sentenceType,
        subject,
        tense,
        fiveSentencePattern,
  ...
  private static generateBeVerb(
    ...
    if (pattern === 'SV') {
      ...
      complement = this.formatBeComplement(
        beComplement,
        subject,
        numberForm,
        nounWords
    ...
    if (tense === 'future') {
      if (sentenceType === 'positive')
        return `${subjectText} will be ${complement}`;
      if (sentenceType === 'negative')
        return `${subjectText} won't be ${complement}`;
      if (sentenceType === 'question')
        return `will ${subjectText} be ${complement}`;
    }
    ...
    if (sentenceType === 'positive') {
      ...
      if (tense === 'past') {
        if (subject === 'first_s' || subject === 'third_s')
          return `${subjectText} wasn't ${complement}`;
  ...
  private static formatBeComplement(
    base: string,
    subject: Subject,
    numberForm: NumberForm,
    nounWords: Word[]
    ...
    const isPluralSubject =
      subject === 'first_p' || subject === 'third_p' || subject === 'second_p';
  ...
  private static generateDoVerb(
    sentenceType: SentenceType,
    subject: Subject,
    tense: Tense,
    verbBase: string,
    pattern: FiveSentencePattern = 'SVO',
    object: Object = 'something',
    numberForm: NumberForm = 'a',
    verbWords: Word[] = []
  ): string {
    const subjectText = this.getSubjectText(subject);
    let complement = this.getPatternComplement(
      pattern,
      subject,
      object,
      numberForm
    ...
    if (patternStr === 'SV' || patternStr === 'DO_SV') {
      const foundVerb = verbWords.find(
        (v) => v.value.toLowerCase() === verbBase.toLowerCase()
      );
    ...
    if (tense === 'future') {
      if (sentenceType === 'positive')
        return `${subjectText} will ${verbBase}${complement ? ' ' + complement : ''}`;
      if (sentenceType === 'negative')
        return `${subjectText} won't ${verbBase}${complement ? ' ' + complement : ''}`;
      if (sentenceType === 'question')
        return `Will ${subjectText} ${verbBase}${complement ? ' ' + complement : ''}`;
    }
    ...
    if (sentenceType === 'positive') {
      ...
      if (tense === 'past')
        return `${subjectText} didn't ${verbBase}${complement ? ' ' + complement : ''}`;
      if (tense === 'present') {
        if (subject === 'third_s')
          return `${subjectText} doesn't ${verbBase}${complement ? ' ' + complement : ''}`;
        return `${subjectText} don't ${verbBase}${complement ? ' ' + complement : ''}`;
      }
    } else if (sentenceType === 'question') {
      if (tense === 'past')
        return `Did ${subjectText} ${verbBase}${complement ? ' ' + complement : ''}`;
      if (tense === 'present') {
        if (subject === 'third_s')
          return `Does ${subjectText} ${verbBase}${complement ? ' ' + complement : ''}`;
  ...
  private static getPatternComplement(
    pattern: FiveSentencePattern,
    subject: Subject,
    object: Object = 'something',
    numberForm: NumberForm = 'a'
    ...
    switch (pattern) {
      ...
      case 'SVO':
        if (numberForm === 'none') {
          ...
          ['my', 'our', 'your', 'his', 'her', 'their'].includes(numberForm)
        ) {
  ...
  private static getThirdPersonForm(verb: string, verbWords: Word[]): string {
    ...
    if (
      verb.endsWith('y') &&
      !['a', 'e', 'i', 'o', 'u'].includes(verb.charAt(verb.length - 2))
    ) {
