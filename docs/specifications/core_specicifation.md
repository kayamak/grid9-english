...
## 8. トップページメニュー構成 (Home Page Menu Structure)
ユーザーが目的の練習モードへ迷わず辿り着けるよう、コマンド選択形式の階層型メニューを採用しています。

- **1階層目 (コマンド領域)**:
    - **たたかう**: 学習メインコンテンツへ。
    - **じゅもん**: 特定の構文パターンを個別に練習。
    - **どうぐ**: 自由な練習モードへ。
- **2階層目 (詳細領域)**:
    - 1階層目で選択したカテゴリーに応じた具体的な練習メニューがリンク付きで表示されます。
    - 例: 「たたかう」を選択 → 「ドリルクエスト」へのリンクを表示。
...
- 2025-12-27 (追記): ホーム画面の階層型メニュー構成を反映。
