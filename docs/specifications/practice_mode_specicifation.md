...
### 8.3 UI/UX の要件
- **トップ画面**: 「コマンド」ウィンドウ内からカテゴリーを選択し、詳細領域に表示される「ドリルクエスト」から開始します。
- **進捗表示**: 現在のレベル、正解数、残り時間、及び 10 個のドットによる進捗（正解：緑、不正解：赤、現在：黄）をリアルタイムに表示します。
- **状態遷移**:
    - **Playing**: 問題に挑戦中。タイムアップまたは正解で次の問題へ進むボタンが表示される。
    - **Result (Success)**: 8問以上正解で表示。次のレベルへ進むボタンが表示。
    - **Failed**: 8問未満で表示。リトライまたはタイトルへ戻るボタンが表示。
    - **All Cleared**: レベル10をクリアすると「GRAND MASTER」として称えられる。
- **演出**: 
    - RPG的なノートブック風 UI に、動的なアニメーション（タイマーゲージ、正解時の緑色発光等）を適用します。
    - **祝杯アニメーション**: レベル10クリア後に、画面全体で紙吹雪が舞い、勝利の祝杯（ジョッキ等のアイコン）が動的に表示される特別なアニメーションを表示します。

## 9. 音響 (Sound & Music)
各モードに応じたBGM（背景音楽）と効果音を再生します。
...
- **再生ルール**:
  - ページロード時またはモード切替時に再生を開始します。
  - ループ再生 (Loop) を有効にします。
  - 音量は控えめ（例: 20%）に設定し、SE（効果音）を阻害しないようにします。
