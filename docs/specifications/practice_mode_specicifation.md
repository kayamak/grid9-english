...
| 要素 | UI要素 | 操作 | 動作 | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| **文型** | `fiveSentencePattern` (タブ) | タブ（「SVC」または「SVO」）をクリックする。| 1. `fiveSentencePattern` のPropsがクリックされたマスに対応する値に更新される。2. これに基づき、**9マスパネルの表示**が更新される。3. **対応する文章（英文の出だし）**が生成・表示される。 | 本タブは動詞タイプが「Do動詞」のタブの配下のみにあるタブです。動詞タイプタブの直下に表示される文型タブです|
